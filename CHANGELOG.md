...
### Changed
- 첫 화면을 블루·라임·흰색·검정 조합으로 다시 구성했다. 실제 음원의 주파수 지형과 A/B 재생 도구를 중심에 두고 모바일 배치, 한글 문구, 앱 아이콘과 저장 이미지까지 맞췄다.
- LINE Seed KR Bold를 제목에, Pretendard를 본문에 적용했다. 필요한 문자만 묶어 로컬에서 제공한다.
- README를 실제 화면과 사용 흐름 중심으로 바꿨다. 2026년 9월 22일에 확인한 디자이너 사례·색상표·공식 서체 자료와 적용 범위를 남겼다.
- 스크립트 지연 실행, 텍스트 압축, 첫 화면 데이터 축소로 초기 전송량을 줄였다. 설치 안내가 본문을 밀어내지 않도록 고쳤다.

### Added
- 샘플 반복 재생과 A·B 단축키. 두 곡의 같은 재생 위치를 실선·점선으로 함께 표시하고, 시간 사이의 움직임을 부드럽게 연결한다.
- 파형 크기 조절, 분석 초기화, 늦게 끝난 디코딩, 오디오 메모리 정리를 확인하는 프런트엔드 회귀 테스트 7개.
- 직접 만든 9초짜리 샘플 세 곡. 재생 위치를 유지하는 A/B 전환, 음색·리듬 비교, 실제 파형·주파수·특성 그래프와 샘플 분석을 제공한다.
- 비교 화면의 여섯 항목 막대그래프와 첫 방문용 샘플 비교. 값을 좋고 나쁨으로 표시하던 표현을 상대적인 크기 비교로 바꿨다.
- 샘플 음원·표시값·분석 엔진의 일치 여부와 압축 후 오디오 Range 응답을 검증하는 테스트.

### Fixed
- 업로드 파형을 기다리지 않고 추천 결과를 바로 보여준다. 새 분석·초기화 때 이전 재생과 파형 작업을 정리해 늦은 작업이 화면을 덮지 못하게 했다.
- 작은 화면에서 업로드 파형 끝이 잘리던 문제를 수정했다. 오른쪽 채널에만 있는 소리와 파일의 마지막 샘플도 파형에 반영한다.
- 휴대전화 공유 버튼이 첫 화면 주소만 보내던 문제를 수정했다. 결과 복사에도 같은 결과 링크를 넣고, 공유 실패를 복사 성공으로 알리지 않도록 했다.
- 분석을 연달아 요청했을 때 로딩 타이머가 겹치지 않도록 했다. 실제 진행 단계처럼 바뀌던 개발 용어를 간단한 대기 안내로 바꿨다.
- 주파수 지형의 면이 다른 곡의 선을 가리던 문제를 수정했다. 화면 폭에 맞춰 선의 밀도를 줄이고, 시점을 바꿔도 그래프와 축이 캔버스 안에 들어오도록 조정했다.
- 공유 링크에서 큰 스펙트로그램을 제외해 주소 길이를 줄였다. 압축 기능이 없는 브라우저에서 만든 링크도 새 브라우저에서 복원한다.
- PNG 저장 시 생성한 SVG가 이미지 보안 정책에 막히던 문제를 수정했다. 기존 CSP는 유지한다.
- 카탈로그의 추천 곡에서 이어 찾은 결과도 분석 기록과 공유 링크에 반영한다.
- 음원은 재생할 때 요청하며 서비스 워커가 부분 재생 요청을 전체 응답으로 바꾸지 않도록 했다.
- 서비스 워커 캐시를 갱신하고 SoundMatch 외의 캐시는 삭제하지 않도록 범위를 좁혔다.

## [1.9.0] — 2026-09-18

메인 화면을 처음부터 다시 그린 릴리즈. 유리 효과와 카드 그림자로 채웠던 화면을
걷어내고, 종이 위에 잉크와 코랄 한 색으로 찍은 인쇄물처럼 다시 설계했다. 음악
유사도 서비스답게 실제 데이터로 그린 시각 자료(소리 지도, 실제 비교 예시, 지표
막대)를 첫 화면부터 보여주고, 번역투였던 문안은 전부 한국어로 다시 썼다.

### Added
- 소리 지도: 카탈로그 781곡을 서버와 같은 StandardScaler 공간에서 PCA 3축으로
  투영해 캔버스에 그린다. 스크롤에 따라 천천히 돌고, 점 위에 올리면 곡명이,
  누르면 그 곡을 기준으로 바로 탐색한다. 분석 결과가 뜨면 기준 곡과 닮은 곡을
  지도 위에 비춘다. 화면에 보일 때만, 바뀐 게 있을 때만 다시 그리고 유휴 회전은
  30fps 로 제한한다. `prefers-reduced-motion` 이면 돌리지 않는다.
- 실제 결과 예시 3쌍: 지금 서버가 쓰는 엔진이 카탈로그 안에서 계산한 결과(유사도,
  6개 요약 지표 비교, 닮은 이유)를 그대로 싣는다. 제목만 다른 동일 음원(100%)
  쌍은 예시에서 뺀다.
- 원리 섹션: 특성 추출 → 표준화·코사인 유사도 → 이유 설명 세 단계를 예시 곡의
  실제 값으로 보여주고, 단계가 화면에 들어오면 오른쪽 지도의 상태가 따라 바뀐다.
- `scripts/build_landing_data.py`: 위 데이터를 `frontend/js/landing-data.js` 로
  생성한다. 카탈로그를 바꾸면 다시 실행한다.
- `/landing.js`, `/landing-data.js` 루트 라우트와 서비스 워커 프리캐시 항목.

### Changed
- 디자인 시스템 교체: 종이 `#F3F1EC` · 잉크 `#121110` · 코랄 `#E8452B`(다크에서는
  `#FF665A`) 세 색과 1px 괘선만으로 화면을 나눈다. 카드 그림자, backdrop-filter,
  배경 그라데이션, 색 테두리를 모두 없앴고 회귀 테스트로 다시 못 들어오게 막았다.
- 라이트 테마가 기본. 다크는 종이와 잉크를 뒤집은 별도 세트로 다시 맞췄다.
- 글꼴을 Pretendard Variable(동적 서브셋)과 IBM Plex Mono 두 종류로 정리했다.
  정적 전체 폰트 대신 쓰는 글자 조각만 받아 첫 로드가 가벼워졌다.
- 모든 문안을 한국어 원문 기준으로 다시 썼다(해요체, 명사구 라벨). 영어는 그
  번역이다. 언어 토글 라벨도 `English` / `한국어` 로 줄였다.
- 결과 화면: 순위 목록을 괘선 행으로, 요약 지표를 괘선 격자로, 레이더와 지표
  막대를 기준 곡=잉크 / 닮은 곡=코랄 두 시리즈로 통일했다. 카탈로그 시드 결과에는
  걸린 시간 대신 곡 수만 보여준다.
- 멜 스펙트로그램과 SVG/PNG 내보내기도 같은 종이·코랄·잉크 팔레트를 쓴다.
- 카탈로그·비교·약관·개인정보·404·오프라인 페이지의 헤더, 폰트, 배경, 푸터를
  메인과 맞췄다.
- 브랜드 마크는 단색 공명 궤적 두 개로 다시 그린 것을 유지하고, SVG·PNG·ICO·iOS·
  PWA·마스커블 아이콘 세트를 같은 마크로 쓴다. BI 가이드(`docs/brand/README.md`)
  는 종이 위의 잉크 원칙으로 다시 썼다.
- README 를 실제 화면 캡처와 동작 원리 그림 중심으로 다시 썼다.
- 서비스 워커 셸 캐시를 `soundmatch-v19` 로 올렸다.

### Tests
- 소리 지도 데이터(781곡 좌표, 예시 3쌍) 형식, 새 랜딩 배선, 종이·잉크·코랄
  팔레트와 유리 효과 금지, 한국어 대표 문구를 고정하는 회귀 테스트를 추가했다.
- 스펙트로그램 색 테스트를 새 팔레트(종이→코랄→잉크)로 갱신했다.
- 1440×900 데스크톱, 390×844 모바일, 라이트·다크 테마에서 메인·결과·카탈로그·비교
  화면을 실제 브라우저로 렌더링해 겹침·넘침·콘솔 오류가 없는지 확인했다.

## [1.8.22] — 2026-07-16

웹 프레임워크와 테스트 도구를 현재 지원 버전으로 맞추고, 신규 설치와 기존
개발 환경에서 같은 조합을 재현하도록 의존성 경계를 정리한 유지보수 릴리즈.

### Changed
- FastAPI를 `0.139.0`, Uvicorn을 `0.51.0`, scikit-learn을 `1.9.0`,
  SoundFile을 `0.14.0`, setuptools를 `83.0.0`으로 올렸다. NumPy는 Python
  3.11 지원을 유지하기 위해 `2.4.6`을 그대로 사용한다.
- pytest `9.1.1`, pytest-asyncio `1.4.0`, pytest-cov `7.1.0`, Ruff
  `0.15.21` 조합으로 개발 도구를 갱신했다.
- Starlette `TestClient`의 우선 HTTP 클라이언트를 유지보수 중인 HTTPX2
  `2.7.0`으로 전환해 구형 HTTPX 사용 중단 예정 경고를 없앴다.
- CI와 릴리즈 워크플로의 `actions/checkout`을 v7로 올렸다.

### Fixed
- 백엔드가 Starlette API를 직접 쓰면서도 버전 선택을 FastAPI의 넓은 허용
  범위에 맡기던 문제를 정리했다. Starlette `1.3.1`을 직접 고정해 신규 설치와
  기존 개발 환경이 같은 웹 프레임워크 조합을 사용한다.
- CI 설치 명령을 `python -m pip`로 통일하고 `pip check` 단계를 추가해,
  테스트 시작 전에 깨진 의존성 그래프를 바로 찾도록 했다.

### Tests
- 검증을 마친 런타임·개발 의존성 핀이 병합 과정에서 되돌아가지 않는지 확인하는
  배포 설정 회귀 테스트를 현재 릴리즈 조합으로 확장했다.
- Python 3.11 신규 가상환경에서 전체 의존성 설치와 `pip check`를 확인하고,
  승인된 로컬 환경에서 린트와 364개 전체 테스트를 통과시켰다.

## [1.8.21] — 2026-07-16

기존 음표·보라색 그라데이션 중심의 시각 체계를 걷어내고 SoundMatch의 제품
의미에서 출발한 BI와 파비콘 체계로 전면 교체한 디자인 릴리즈.

### Added
- “정확히 같지는 않지만 닮음”을 나타내는 근삿값 기호 `≈`를 두 개의 서로 다른
  획으로 다시 그려 SoundMatch 전용 브랜드 마크를 만들었다.
- SVG 파비콘과 함께 32px PNG, 멀티사이즈 ICO, iOS 180px, PWA 192/512px,
  Android 마스커블 아이콘을 용도별 안전 영역에 맞춰 제작했다.
- 색상 역할, 로고 사용 규칙, 원본 자산 위치를 정리한 한국어 BI 가이드를
  `docs/brand/README.md` 에 추가했다.
- 백엔드와 디자인 프리뷰 서버에 PNG/ICO 파비콘 루트 경로를 추가했다.

### Changed
- 헤더 워드마크를 소문자 `soundmatch`로 정리하고 메인·카탈로그·비교·약관·
  개인정보 페이지에서 같은 브랜드 마크를 사용한다.
- 기본 화면을 `#101010` / `#efefef` 중심의 편집 디자인 톤으로 바꾸고,
  `#b9ee84`는 핵심 행동과 1순위 결과, `#b9e0fd`는 비교 데이터,
  `#faed27`은 주의 상태에만 쓰도록 역할을 분리했다.
- 배경의 보라·청록 블러 오브와 카드 그라데이션을 제거하고 단색 면, 얇은 선,
  미세 노이즈로 화면 깊이를 잡았다.
- OpenGraph 카드, 결과 이미지 내보내기, 멜 스펙트로그램도 같은 BI 색상과
  `≈` 마크를 사용하도록 맞췄다.
- 서비스 워커 셸 캐시를 `soundmatch-v16`으로 올리고 새 파비콘 자산을
  프리캐시 목록에 포함했다.

### Tests
- SVG/PNG/ICO 파비콘 노출, 파일 시그니처, 픽셀 크기, 프리뷰 서버 별칭을
  검증하는 회귀 테스트를 추가했다.
- 파비콘이 평면 색상과 두 획 구조를 유지하고 예전 음표 그라데이션 및 배경
  오브가 다시 들어오지 않는지 확인하는 BI 회귀 테스트를 추가했다.

## [1.8.20] — 2026-07-15

분석 결과의 레이더 차트를 실제 데스크톱·모바일 브라우저에서 점검해 축 라벨
잘림과 언어 불일치를 정리한 패치 릴리즈.

### Fixed
- 숨긴 카드의 `clientWidth` 를 읽어 320px 폴백 좌표로 그리던 순서를 바꿔,
  레이더 차트가 현재 화면의 실제 너비를 기준으로 렌더링되게 했다.
- SVG 축 라벨의 실제 글자 폭을 측정해 필요한 만큼 `viewBox` 여백을 확보한다.
  390px 모바일 화면에서 `Chroma`, `Harmony`, `Brightness` 라벨이 좌우로
  잘리던 문제를 함께 해결했다.
- 레이더 차트의 여섯 축 라벨과 `aria-label` 이 한국어/영어 전환을 따라가도록
  번역 키를 연결했다.
- 서비스 워커 셸 캐시를 `soundmatch-v14` 로 올리고 프리캐시 요청에 빌드별
  쿼리를 붙였다. 업데이트 중 이전 워커가 요청을 가로채더라도 구 버전
  `app.js`, `i18n.js`, `visualizers.js` 를 새 캐시에 다시 넣지 못한다.

### Tests
- 레이더 카드를 표시한 뒤 실제 너비를 측정하는 순서, SVG 라벨 경계 계산,
  한·영 축 라벨/대체 텍스트 연결을 확인하는 정적 회귀 테스트를 추가했다.
- 서비스 워커가 버전 쿼리로 새 셸 자산을 받고 canonical 경로에 저장하는지
  확인하는 캐시 갱신 회귀 테스트를 추가했다.
- Playwright CLI 로 390×844, 1440×900 화면을 직접 렌더링하고 라벨의
  bounding rect 가 SVG 경계 안에 들어오는지 확인했다.

## [1.8.19] — 2026-07-15

v1.8.18 이후 실제 화면 캡처 중심의 README 개편과 Dependabot 의존성 갱신을
하나의 배포 단위로 묶은 패치 릴리즈. 제품 소개는 더 보기 좋아졌지만 운영자가
따라가야 할 릴리즈/환경 변수 문서가 빠졌던 부분도 같이 되살렸다.

### Changed
- README 를 메인, 결과, 카탈로그, 비교, 라이트/모바일 화면 캡처 중심으로
  다시 정리해 처음 보는 사람도 서비스 흐름을 바로 이해할 수 있게 했다.
- `python-multipart` 런타임 의존성을 `0.0.32` 로, `pytest` / `pytest-asyncio`
  개발 의존성을 각각 `9.0.3` / `1.3.0` 으로 올린 변경을 이번 릴리즈 범위에
  포함했다.
- README 에 CLI 버전 확인 예시, 핵심 운영 환경 변수, 수동 릴리즈 순서를 다시
  추가해 문서 리라이트 뒤에도 배포 절차를 놓치지 않게 했다.

### Fixed
- `python-multipart` 갱신 병합 과정에서 `uvicorn[standard]` 핀이 `0.49.0` 에서
  `0.27.1` 로 되돌아간 문제를 다시 `0.49.0` 으로 맞췄다.
- `pytest==9.0.3` 과 `pytest-asyncio==0.23.5` 가 동시에 설치되지 않던 CI
  의존성 충돌을 `pytest-asyncio==1.3.0` 으로 맞춰 풀었다.
- `backend.__version__`, OpenAPI 스키마 예시, README CLI 예시가 `1.8.19` 와
  `2026-07-15` 릴리즈 날짜를 함께 가리키도록 맞췄다.

### Tests
- 런타임/개발 의존성 파일이 이번 Dependabot 갱신 버전을 유지하는지 확인하는
  정적 회귀 테스트를 추가했다.

## [1.8.18] — 2026-06-02

v1.8.17 이후 결과 영역의 오디오 재생 컨트롤과 태그 그룹 접근성 라벨을
언어 전환 흐름에 맞춘 패치 릴리즈.

### Fixed
- 업로드 음원 미리듣기 버튼의 기본 `aria-label` 을 `Play / pause` 에서
  `업로드 음원 재생 / 일시정지` 로 바꾸고, i18n 적용 후 현재 언어의
  `shortcuts.playPause` 라벨로 갱신되게 했다.
- 결과 태그 그룹의 `aria-label` 도 `results.tagsLabel` 키를 통해 한국어/영어
  전환을 따라가도록 했다.

### Tests
- 결과 영역 재생 버튼과 태그 그룹이 한국어 기본 라벨 + i18n 속성을 함께
  유지하는지 확인하는 정적 회귀 테스트를 추가했다.

## [1.8.17] — 2026-06-02

v1.8.16 이후 shell 페이지의 테마 토글 접근성 기본 라벨을 한국어 우선
경험에 맞춘 패치 릴리즈.

### Fixed
- 홈, 카탈로그, 비교, 개인정보, 이용약관 페이지의 테마 전환 버튼 기본
  `aria-label` 을 `Toggle theme` 에서 `테마 전환` 으로 바꿨다. i18n 로드
  이후에는 기존처럼 현재 언어의 `controls.themeToggle` 값으로 갱신된다.

### Tests
- 주요 shell 페이지의 테마 토글이 한국어 기본 라벨과 i18n 속성을 함께
  유지하는지 확인하는 정적 회귀 테스트를 추가했다.

## [1.8.16] — 2026-06-02

v1.8.15 이후 shell 내비게이션의 접근성 라벨을 한국어/영어 전환 흐름에 맞춘
패치 릴리즈. 화면 텍스트는 이미 번역됐지만, 보조기기 landmark 이름이 영어
고정값으로 남아 있던 부분을 정리했다.

### Fixed
- 홈, 카탈로그, 비교, 개인정보, 이용약관 페이지의 주요 내비게이션
  `aria-label` 을 `Primary` 고정값에서 `주요 메뉴` 기본값 + i18n 속성으로
  바꿨다. 언어를 영어로 전환하면 `Primary navigation` 으로 함께 갱신된다.

### Tests
- 주요 shell 페이지의 nav landmark 라벨이 `nav.primary` i18n 키를 사용하고,
  `Primary` 하드코딩으로 되돌아가지 않는지 확인하는 정적 회귀 테스트를
  추가했다.
