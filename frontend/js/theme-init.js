// 페이지가 처음 페인트되기 전에 저장된 테마(라이트/다크) 를 즉시 적용한다.
...
//
// 기본은 라이트(종이). 사용자가 토글한 적이 없고 OS 가 다크를 선호할 때만 다크.
  ...
  try {
    ...
    if (!stored && window.matchMedia) {
      ...
      var handler = function (e) {
        ...
        // 파형 다시 그리기처럼 테마 변경에 반응하는 코드가 있을 수 있으니 알려준다.
        try { window.dispatchEvent(new CustomEvent("theme:change", { detail: { theme: next, source: "system" } })); } catch (err) {}
      };
    ...
    // localStorage 가 막혀 있어도 무시. 기본 라이트 테마 그대로.
